 ...
 *
 * Connects to any running MCP server (generated by `stellar mcp generate`),
 * discovers available tools, calls them, and signs/submits transactions
 * via pluggable signer adapters.
 *
 * @example
 * ```ts
 ...
 * const { xdr } = await client.call('deploy-token', { admin: 'GABC...' });
 * const result = await client.signAndSubmit(xdr!, { signer: secretKeySigner(key) });
 * client.close();
 * ```
 */
export class MCPClient {
   ...
   *
   * For write operations (deploy, transfer, etc.), the result includes `xdr`
   * and optionally `simulationResult`. For read-only tools, only `data` is populated.
   */
  async call(toolName: string, args: Record<string, unknown> = {}): Promise<CallResult> {
