...
import { isReadOperation, extractArgs, buildToolArgs, parseArgValue, argKey, secretKeySigner, MCPConnectionError, MCPToolError, } from '@stellar-mcp/client';
import { createClient, canSign } from '../mcp.js';
import { config } from '../config.js';
import { INTERNAL_TOOLS, SPINNER_CONNECTING, SPINNER_CALLING, SPINNER_SIGNING, } from '../constants.js';
import { mkSpinner, printResult, printSuccess, printWritePreview, printError } from '../ui.js';
export async function handleCall(toolArg, opts) {
...
// ─── Shared execute path ──────────────────────────────────────────────────────
export async function executeAndDisplay(toolName, args, jsonMode) {
    const client = createClient();
...
async function collectArgs(tool) {
    const argDefs = extractArgs(tool);
    const collected = {};
    let cancelled = false;
    await p.group(Object.fromEntries(argDefs.map((arg) => {
...
function buildArgsFromFlags(tool, flags) {
    const argDefs = extractArgs(tool);
    const collected = {};
