...
export function mkSpinner(msg: string): ReturnType<typeof ora> {
...
export function printToolsTable(readTools: ToolInfo[], writeTools: ToolInfo[]): void {
...
export function printResult(toolName: string, data: unknown): void {
...
function colorizeJson(json: string): string {
  ...
  let result = json.replace(
    /"([^"]+)":\s*"([^"]*)"/g,
    (_, k: string, v: string) => `${chalk.cyan(`"${k}"`)}: ${chalk.green(`"${v}"`)}`,
  );
  // Replace remaining keys (non-string values)
  result = result.replace(
    /"([^"]+)":/g,
    (_, k: string) => chalk.cyan('"' + k + '"') + ':',
  );
  // Gray numbers and booleans
  result = result.replace(/: (true|false)/g, (_, v: string) => `: ${chalk.yellow(v)}`);
  result = result.replace(/: (-?\d+(?:\.\d+)?)/g, (_, v: string) => `: ${chalk.magenta(v)}`);
  return result;
}
...
export function printSuccess(hash: string): void {
  console.log();
...
export function printWritePreview(xdr: string): void {
  console.log();
